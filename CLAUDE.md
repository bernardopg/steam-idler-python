...
3. **Drops remaining** (`filter_completed_card_drops`) — `_filter_completed_card_drops()` prefers `BadgeService` (API badges), falling back to `CardDropChecker` (authenticated community-page scraping). Needs a steam_id; skipped otherwise.
