...
def test_cache_persistence_and_reload(tmp_path: Path, monkeypatch):
    ...
    det = TradingCardDetector(cache_enabled=True, cache_path=str(cache_file), cache_ttl_days=30)
    ...
    det2 = TradingCardDetector(cache_enabled=True, cache_path=str(cache_file), cache_ttl_days=30)
...
def test_cache_ttl_expiry(tmp_path: Path, monkeypatch):
    cache_file = tmp_path / "cards.json"
    det = TradingCardDetector(cache_enabled=True, cache_path=str(cache_file), cache_ttl_days=1)
    ...
    det2 = TradingCardDetector(cache_enabled=True, cache_path=str(cache_file), cache_ttl_days=1)
